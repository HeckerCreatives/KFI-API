...
exports.exportAgingOfLoansExcel = (datas, loanCodes, from = "", to = "") => {
  ...
  agingOfLoans.push([
    { v: `AO`, t: "s", s: { font: { bold: true }, alignment: { vertical: "center" }, border: topBottomBorder } },
  ...
  datas.map(data => {
    ...
    agingOfLoans.push([
      { v: `${data?.client?.center?.acctOfficer || ""}`, t: "s", s: { font: { bold: true }, alignment: { vertical: "center" } } },
  ...
  const worksheet = XLSX.utils.aoa_to_sheet(agingOfLoans, { origin: "A7" });
  ...
  const headerSubtitle = `Loan Release Aging Of Loans`;
