...
exports.lrExportByAccounts = datas => {
  ...
  datas.map(data => {
    formattedLoanReleases.push([{ v: `Account:  ${data.code} - ${data.description}`, t: "s", s: { alignment: { vertical: "center" } } }]);
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        formattedLoanReleases.push([
          { v: entry?.client ? `${entry?.transaction?.acctOfficer} - ${entry?.client?.name}` : "", t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${completeNumberDate(entry.transaction.date)}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry.transaction.code}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry.center.centerNo} - ${entry.center.description}`, t: "s", s: { alignment: { vertical: "center" } } },
