...
exports.evExportByAccounts = datas => {
  const rows = [];
  ...
  datas.map(data => {
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          { v: `${entry?.client?.name || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${completeNumberDate(entry.expense.date)}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry.expense.code}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry?.expense?.remarks || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Expense Voucher By Accounts (Sort By Supplier)`;
