...
exports.evPrintDetailedByAccounts = (datas, from = "", to = "") => {
  const info = { title: "Expense Voucher" };
  ...
  datas.map(data => {
    ...
    data.entries.map(entry => {
      ...
      const isAdded = expenses.findIndex(e => e.name === name);
      if (isAdded < 0) expenses.push({ name, entries: [entry] });
      if (isAdded >= 0) expenses[isAdded].entries.push(entry);
    });
    ...
    expenses.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          { text: entry?.client ? `${entry?.expense?.acctOfficer} - ${entry?.client?.name}` : "", fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${completeNumberDate(entry.expense.date)}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${entry.expense.code}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${entry?.expense?.remarks || ""}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
  ...
  const contents = [
    ...
    { text: "Expense Voucher By Account ( Sort By Supplier )", fontSize: 9 },
