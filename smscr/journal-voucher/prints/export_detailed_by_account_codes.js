...
exports.jvExportByAccounts = datas => {
  const rows = [];
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { v: `${entry?.journal?.remarks || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Journal Voucher By Accounts (Sort By Supplier)`;
