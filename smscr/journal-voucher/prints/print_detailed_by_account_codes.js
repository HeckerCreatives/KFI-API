...
exports.jvPrintDetailedByAccounts = (datas, from = "", to = "") => {
  const info = { title: "Journal Voucher" };
  ...
  datas.map(data => {
    journalVouchers.push([{ text: `Account:   ${data.code} - ${data.description}`, colSpan: 6, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] }, {}, {}, {}, {}, {}]);
    let journals = [];
    let astDebit = 0;
    let astCredit = 0;

    data.entries.map(entry => {
      let name = entry?.client ? `${entry?.journal?.acctOfficer} - ${entry?.client?.name}` : "NotApplicable";
      const isAdded = journals.findIndex(e => e.name === name);
      if (isAdded < 0) journals.push({ name, entries: [entry] });
      if (isAdded >= 0) journals[isAdded].entries.push(entry);
    });
    ...
    journals.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        journalVouchers.push([
          { text: entry?.client ? `${entry?.journal?.acctOfficer} - ${entry?.client?.name}` : "", fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${completeNumberDate(entry.journal.date)}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${entry.journal.code}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${entry?.journal?.remarks || ""}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
      ...
      journalVouchers.push([
        ...
        { text: ``, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
        { text: ``, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
        { text: `Payee Sub-Total: `, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0], alignment: "right" },
    ...
    journalVouchers.push([...Array.from({ length: 6 }, () => ({ text: ``, fontSize: 8, margin: [0, 5, 0, 5], border: [0, 0, 0, 0] }))]);
    journalVouchers.push([
      { text: ``, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
      { text: ``, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
      { text: ``, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
  ...
  const contents = [
    ...
    { text: "Journal Voucher By Account ( Sort By Supplier )", fontSize: 9 },
      ...
      table: {
        ...
        body: [
          [
            ...
            { text: "Particulars", fontSize: 8, bold: true, margin: [0, 4.5, 0, 4.5], border: [0, 1, 0, 1], alignment: "left" },
