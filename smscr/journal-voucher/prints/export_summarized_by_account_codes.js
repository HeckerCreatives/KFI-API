...
exports.jvExportSummarizedByAccounts = datas => {
  const rows = [];
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { v: `${completeNumberDate(entry.journal.date)}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry.journal.code}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry?.journal?.nature || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Journal Voucher By Accounts (Sort By Client)`;
