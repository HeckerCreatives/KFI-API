...
exports.relExportByAccountsDetailed = (datas, from, to) => {
  const rows = [];
  ...
  datas.map(release => {
    ...
    rows.push([{ v: `${release.code} - ${release.description}`, t: "s", s: { font: { bold: true }, alignment: { vertical: "center" } } }]);
    ...
    release.entries.map(data => {
      ...
      rows.push([
        ...
        { v: `${completeNumberDate(data.release.date)}`, t: "s", s: { alignment: { vertical: "center" } } },
        { v: `${data.release.code}`, t: "s", s: { alignment: { vertical: "center" } } },
        { v: `${data.release.center.centerNo} - ${data.release.center.description}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Acknowledgement Receipt By Accounts ( Detailed )`;
