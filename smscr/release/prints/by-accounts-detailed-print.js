...
exports.relPrintByAccountsDetailed = (datas, from = "", to = "") => {
  ...
  datas.map(release => {
    ...
    rows.push([{ text: `${release.code} - ${release.description}`, colSpan: 6, fontSize: 8, margin: [0, 1, 0, 1], border: [0, 0, 0, 0] }, {}, {}, {}, {}, {}]);
    release.entries.map(data => {
      ...
      rows.push([
        ...
        { text: `${completeNumberDate(data?.release.date)}`, fontSize: 8, margin: [0, 1, 0, 1], border: [0, 0, 0, 0] },
        { text: `${data?.release.code}`, fontSize: 8, margin: [0, 1, 0, 1], border: [0, 0, 0, 0] },
        { text: `${data.release.center.centerNo} - ${data.release.center.description}`, fontSize: 8, margin: [0, 1, 0, 1], border: [0, 0, 0, 0] },
  ...
  const contents = [
    ...
    { text: "Acknowledgement Receipt By Account Code ( Detailed )", fontSize: 9 },
