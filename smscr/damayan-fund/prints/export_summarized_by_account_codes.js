...
exports.dfExportSummarizedByAccounts = datas => {
  const rows = [];
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { v: `${entry?.damayan?.nature || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Damayan Fund By Accounts (Sort By Client)`;
