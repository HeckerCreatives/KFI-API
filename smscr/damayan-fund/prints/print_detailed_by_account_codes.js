...
exports.dfPrintDetailedByAccounts = (datas, from = "", to = "") => {
  const info = { title: "Damayan Funds" };
  ...
  datas.map(data => {
    ...
    data.entries.map(entry => {
      ...
      const isAdded = damayans.findIndex(e => e.name === name);
      if (isAdded < 0) damayans.push({ name, entries: [entry] });
      if (isAdded >= 0) damayans[isAdded].entries.push(entry);
    });
    ...
    damayans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { text: `${completeNumberDate(entry.damayan.date)}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${entry.damayan.code}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
          { text: `${entry?.damayan?.remarks || ""}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
  ...
  const contents = [
    ...
    { text: "Damayan Fund By Account ( Sort By Supplier )", fontSize: 9 },
