...
exports.elExportByAccounts = datas => {
  const rows = [];
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { v: `${entry?.emergency?.remarks || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Emergency Loan By Accounts (Sort By Supplier)`;
