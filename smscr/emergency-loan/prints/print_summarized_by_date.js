...
exports.elPrintSummarizedByDate = (datas, from = "", to = "") => {
  const info = {
  ...
  datas.map(data => {
    ...
    data.emergencies.map(damayan => {
      ...
      emergencyLoans.push([
        ...
        { text: `${completeNumberDate(damayan?.checkDate)}`, fontSize, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
        { text: formatNumber(damayan.amount), fontSize, margin: [0, 0, 0, 0], border: [0, 0, 0, 0], alignment: "right" },
      ]);
    });
  ...
  const contents = [
    ...
    { text: "Emergency Loan By Date ( Summarized )", fontSize: 9 },
