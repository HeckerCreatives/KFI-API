...
exports.elExportSummarizedByAccounts = datas => {
  const rows = [];
    ...
    loans.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { v: `${completeNumberDate(entry.emergency.date)}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry.emergency.code}`, t: "s", s: { alignment: { vertical: "center" } } },
          { v: `${entry?.emergency?.nature || ""}`, t: "s", s: { alignment: { vertical: "center" } } },
  ...
  const headerSubtitle = `Emergency Loan By Accounts (Sort By Client)`;
