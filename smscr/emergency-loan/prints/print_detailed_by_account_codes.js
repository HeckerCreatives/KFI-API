...
exports.elPrintDetailedByAccounts = (datas, from = "", to = "") => {
  const info = { title: "Emergency Loans" };
    ...
    emergencies.map(loan => {
      ...
      loan.entries.map(entry => {
        ...
        rows.push([
          ...
          { text: `${entry?.emergency?.remarks || ""}`, fontSize: 8, margin: [0, 0, 0, 0], border: [0, 0, 0, 0] },
  ...
  const contents = [
    ...
    { text: "Emergency Loan By Account ( Sort By Supplier )", fontSize: 9 },
